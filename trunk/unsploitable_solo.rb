...
module Msf
	...
	class Plugin::UnsploitableSolo < Msf::Plugin
		...
		def on_session_open(session)
			...
			if (session.interactive?)
				...
				print_status("Unsploitable Solo Initiated: #{host} - #{session.via_exploit}")
				...
				session.init_ui(session.input, Rex::Ui::Text::Output::Stdio.new)
				print_status("Unsploitable Solo Complete, Enjoy Your Session")
				return
			end
		...
		def initialize(framework, opts)
			...
			print_status("Unsploitable Solo: Loaded")
		end
		...
		def desc
			"Automatically Patch Metasploitable Vulnerabilities - Solo"
		end
